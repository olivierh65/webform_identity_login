...
class IdentityLoginHandler extends WebformHandlerBase {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state, WebformSubmissionInterface $webform_submission) {
    ...
    if ($last_submission) {
      ...
      $form_state->getFormObject()->setEntity($last_submission);

      $pages = $webform_submission->getWebform()->getPages();
      $page_keys = array_keys($pages);
      $current_page = $form_state->get('current_page') ?: $webform_submission->getCurrentPage();
      ...
      if ($next_page) {
        ...
        $last_submission->setCurrentPage($next_page);
        $last_submission->save();

        $logger->debug('last_submission: sid=@s, page=@p, draft=@d, completed=@c', [
          '@s' => $last_submission->id(),
          '@p' => $last_submission->getCurrentPage(),
          '@d' => $last_submission->isDraft() ? 'YES' : 'NO',
          '@c' => $last_submission->isCompleted() ? 'YES' : 'NO',
        ]);
      }
